...
> ⚠️ **PENDIENTE DE AUDITORÍA (2026-09-13).** Los 4 coeficientes de abajo ya eran no significativos bajo el modelo estático sin tendencia (los únicos números de esta sección que se reportan como tal). Al agregar un control de tendencia lineal pre-existente, empleo_temporal y participación_permanente cambian de signo y de significancia -- **cambio de signo bajo control de tendencia, robustez a forma funcional pendiente de verificar -- no interpretar como hallazgo hasta resolver.** No se incluye ningún número de Exposure con control de tendencia en este borrador todavía (a diferencia de Bite en la sección 4.2, donde el efecto significativo del modelo estático SÍ se resolvió: desaparece con tendencia, de forma estable). Nota de terminología: "modelo estático" aquí es una forma funcional (post_2023 como dummy único), **no** la "Especificación A" de la sección 4.5 de la tesis (muestra completa a nivel establecimiento) — ambos scripts corren sobre la muestra completa, sin restricción a multiplanta; el nombre "Especificación A/B" que usaban originalmente los scripts (hasta los commits `64b04ea`/`3b55e37`) era una colisión de nombres accidental, ya corregida en el código.
