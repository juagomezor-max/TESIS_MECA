...
Los scripts están numerados por **orden de lectura** (cómo se lee un estudio
empírico: descriptivos → medidas de exposición → análisis → validaciones →
validaciones adicionales), que ya **no coincide** con el orden de ejecución.
Ver "Orden de ejecución" en `README.md` antes de correr el pipeline desde
cero: en resumen, `02_medidas_exposicion.R` debe correr antes que `03`, `04`
y `05`, aunque se lea después de `01`.
...
4. **El placebo 2018-2019 del primer eslabón no es informativo por
   ...
   2019 → crecimiento 2018-2019 bajo), no diseño. — `03_primer_eslabon_medidas.R`
   (sección 7).
5. **Bite es la medida menos estable entre años base** (Pearson 0,57,
   `04_decision_medida.R`, tabla T03) **y la de menor cobertura en la celda
...
1. **¿Qué mide exactamente "4,80%"?** `07_reconciliacion.R` (punto
   ...
   `01_descriptivos_y_contexto.R` deja una nota que sugiere que son la misma cosa descrita de
...
2. **¿La relación por tramos de exposición se aplana o es monotónica
   ...
   tabla `T04_efecto_por_tramos` de `04_decision_medida.R` antes de escribir
   cualquiera de las dos afirmaciones.**
3. **El criterio de decisión de medida que se eliminó** (el placebo
   2018-2019 como criterio 3 en `03_primer_eslabon_medidas.R`) -- confirmado que ya no se
