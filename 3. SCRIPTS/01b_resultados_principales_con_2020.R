# ==============================================================================
# 01b_resultados_principales_con_2020.R
#
...
# Fecha:   2026-09-17
#
# VALIDACIÓN: es el mismo análisis de 01_resultados_principales.R, pero
# INCLUYENDO 2020. La especificación principal de la tesis sigue excluyendo
# 2020 (pandemia). Este script responde a la pregunta "¿y con 2020 qué pasa?".
#
# Qué cambia frente al script principal:
#   1. Los paneles de 1. DATOS/ no tienen 2020. Construimos las filas de 2020
#      desde la macrobase con las MISMAS fórmulas del panel, y antes lo
#      comprobamos reconstruyendo 2019 y 2021 y comparándolos con el panel.
#   2. Los estudios de evento y la prueba de años previos incluyen 2020.
#   3. En la sección 5, "pandemia" marca 2020 y 2021, y "subsidios" 2020 a 2023.
#   4. Todo se guarda en 4. RESULTADOS/RESULTADOS2020/ (no pisa los resultados principales).
#
...
# Todo va dentro de 4. RESULTADOS/RESULTADOS2020/ para no pisar los resultados principales
RAIZ_RESULTADOS      <- file.path("4. RESULTADOS", "RESULTADOS2020")
CARPETA_DESCRIPTIVOS <- file.path(RAIZ_RESULTADOS, "Descriptivos")
CARPETA_PRINCIPAL    <- file.path(RAIZ_RESULTADOS, "Principal")
CARPETA_ESTIMACION   <- file.path(RAIZ_RESULTADOS, "Estimacion")
CARPETA_VALIDACIONES <- file.path(RAIZ_RESULTADOS, "Validaciones")
CARPETA_ROBUSTEZ     <- file.path(RAIZ_RESULTADOS, "Robustez")
...
# Panel de firmas: una fila por firma y año (2020 no está; lo agregamos en 1.1b)
...
cat("Panel de firmas (sin 2020):", nrow(panel), "filas,", ncol(panel), "columnas,",
    n_distinct(panel$NORDEMP), "firmas\n")

# --- 1.1b Agregamos 2020 ----------------------------------------------------------
# El panel principal no tiene 2020. Hay dos caminos:
#   A. Si existe 1. DATOS/panel_analitico_firma_eam_con_2020.rds (construido con
#      el mismo proceso del panel, desde VS Code), lo usamos directamente y
#      comprobamos que los demás años sean iguales al panel principal.
#   B. Si no existe, construimos 2020 desde la macrobase (abajo).
#
# Camino B: armamos esas filas desde la macrobase sumando las
# plantas de cada firma, con las mismas fórmulas del panel
# (0. ANALISIS INICIAL IA/3. SCRIPTS/pipeline/01_construir_base.R y 03_construir_panel.R):
#   - cada columna se suma entre plantas; si todas las plantas están vacías, queda NA
#   - los grupos de personal se suman con los vacíos como cero
# Antes de usar 2020, reconstruimos 2019 y 2021 con la misma función y los
# comparamos con el panel. Si no coinciden, el script se detiene.

# Si quieres seguir aunque la comparación no coincida (NO recomendado), cambia a FALSE
DETENER_SI_NO_COINCIDE <- TRUE

ruta_panel_con_2020 <- file.path("1. DATOS", "panel_analitico_firma_eam_con_2020.rds")

if (file.exists(ruta_panel_con_2020)) {
  
  # --- Camino A: usamos la base con 2020 ya construida ---------------------------
  cat("Usamos la base con 2020 ya construida:", ruta_panel_con_2020, "\n")
  panel_con_2020 <- read_rds(ruta_panel_con_2020) %>%
    mutate(NORDEMP = as.character(NORDEMP),
           ANIO = as.integer(as.character(ANIO)))
  
  # Comprobación: en los años distintos de 2020 debe ser igual al panel principal
  variables_a_comparar <- intersect(
    c("empleo_total", "empleo_permanente", "empleo_temporal", "participacion_permanente", "VALORVEN",
      "Bite2022_obreros", "costos_totales_personal_total_c3r10c3", "sueldos_permanentes_obreros_c3r2c1",
      "obreros_permanentes", "temporal_directo", "temporal_agencias", "aprendices"),
    intersect(names(panel), names(panel_con_2020)))
  
  comprobacion <- tibble()
  for (anio in setdiff(sort(unique(panel$ANIO)), 2020)) {
    original <- panel %>% filter(ANIO == anio) %>% select(NORDEMP, ANIO, all_of(variables_a_comparar))
    nuevo    <- panel_con_2020 %>% filter(ANIO == anio) %>% select(NORDEMP, ANIO, all_of(variables_a_comparar))
    cruce <- inner_join(original, nuevo, by = c("NORDEMP", "ANIO"), suffix = c("_panel", "_nuevo"))
    for (variable in variables_a_comparar) {
      x <- cruce[[paste0(variable, "_panel")]]
      y <- cruce[[paste0(variable, "_nuevo")]]
      iguales <- (is.na(x) & is.na(y)) | (!is.na(x) & !is.na(y) & abs(x - y) <= pmax(0.5, 1e-6 * abs(x)))
      comprobacion <- bind_rows(comprobacion, tibble(
        anio = anio, variable = variable,
        firmas_en_panel = nrow(original), firmas_reconstruidas = nrow(nuevo), firmas_cruzadas = nrow(cruce),
        porcentaje_iguales = round(100 * mean(iguales), 2)
      ))
    }
  }
  ver(comprobacion, filas = 120)
  
  if (any(comprobacion$porcentaje_iguales < 99) ||
      any(comprobacion$firmas_reconstruidas != comprobacion$firmas_en_panel)) {
    mensaje <- "La base con 2020 NO coincide con el panel principal en los demás años (ver tabla arriba)."
    if (DETENER_SI_NO_COINCIDE) stop(mensaje)
    cat("\nATENCIÓN:", mensaje, "Se sigue porque DETENER_SI_NO_COINCIDE = FALSE.\n")
  } else {
    cat("\nLa base con 2020 coincide con el panel principal en todos los demás años.\n")
  }
  
  cat("Firmas en 2020:", sum(panel_con_2020$ANIO == 2020), "\n")
  panel <- panel_con_2020 %>% arrange(NORDEMP, ANIO)
  
} else {
  
  # --- Camino B: construimos 2020 desde la macrobase -----------------------------
  # Buscamos la macrobase en las ubicaciones donde puede estar
  rutas_macrobase <- c(
    file.path("1. DATOS", "5. MACROBASE", "macro_base_eam.rds"),
    file.path("0. ANALISIS INICIAL IA", "1. DATOS", "5. MACROBASE", "macro_base_eam.rds")
  )
  ruta_macrobase <- rutas_macrobase[file.exists(rutas_macrobase)][1]
  if (is.na(ruta_macrobase)) {
    stop("No encontramos la base con 2020 ni la macrobase. Construye 1. DATOS/panel_analitico_firma_eam_con_2020.rds ",
         "desde VS Code, o busca la macrobase con:\n",
         "  list.files('.', pattern = 'macro_base_eam', recursive = TRUE, full.names = TRUE)")
  }
  cat("Macrobase encontrada en:", ruta_macrobase, "\n")
  macro <- read_rds(ruta_macrobase)
  names(macro) <- toupper(names(macro))
  macro <- macro %>%
    mutate(NORDEMP = as.character(NORDEMP),
           ANIO = as.integer(suppressWarnings(as.numeric(as.character(ANIO)))))
  cat("Macrobase:", nrow(macro), "filas. Años:", paste(sort(unique(macro$ANIO)), collapse = ", "), "\n")
  if (!2020 %in% macro$ANIO) stop("La macrobase no tiene 2020.")
  
  # Grupos de columnas de personal (iguales a 03_construir_panel.R)
  ...
  cols_profesionales   <- c("C4R1C3N", "C4R1C4N", "C4R2C3E", "C4R2C4E", "C4R1C5N", "C4R1C6N", "C4R2C5E", "C4R2C6E",
                            "C4R1C7N", "C4R1C8N", "C4R2C7E", "C4R2C8E", "C4R6MN", "C4R6HN", "C4R6ME", "C4R6HE")
  cols_permanente      <- c("C4R2C1", "C4R2C2", "C4R2C3", "C4R2C4", "C4R1C3N", "C4R1C4N", "C4R2C3E", "C4R2C4E")
  cols_temporal        <- c("C4R3C1", "C4R3C2", "C4R4C1", "C4R4C2", "C4R3C3", "C4R3C4", "C4R4C3", "C4R4C4",
                            "C4R1C5N", "C4R1C6N", "C4R2C5E", "C4R2C6E", "C4R1C7N", "C4R1C8N", "C4R2C7E", "C4R2C8E")
  cols_temp_directo    <- c("C4R3C1", "C4R3C2", "C4R3C3", "C4R3C4", "C4R1C5N", "C4R1C6N", "C4R2C5E", "C4R2C6E")
  cols_temp_agencias   <- c("C4R4C1", "C4R4C2", "C4R4C3", "C4R4C4", "C4R1C7N", "C4R1C8N", "C4R2C7E", "C4R2C8E")
  cols_aprendices      <- c("C4R6OM", "C4R6OH", "C4R6DM", "C4R6DH", "C4R6MN", "C4R6HN", "C4R6ME", "C4R6HE")
  cols_valores         <- c("C3R10C3", "C3R2C1", "C3R2C2", "C3R2PT", "VALORVEN")
  
  columnas_macro <- unique(c(cols_obreros, cols_administrativos, cols_profesionales, cols_permanente,
                             cols_temporal, cols_valores))
  faltan_macro <- setdiff(columnas_macro, names(macro))
  if (length(faltan_macro) > 0) stop("Faltan columnas en la macrobase: ", paste(faltan_macro, collapse = ", "))
  
  # suma_con_na(): suma las plantas; si todas están vacías, deja NA
  suma_con_na <- function(x) if (all(is.na(x))) NA_real_ else sum(x, na.rm = TRUE)
  
  # construir_anio(): arma las filas firma-año de un año desde la macrobase
  construir_anio <- function(anio) {
    macro %>%
      filter(ANIO == anio, !is.na(NORDEMP), NORDEMP != "") %>%
      mutate(across(all_of(columnas_macro), ~ suppressWarnings(as.numeric(as.character(.x))))) %>%
      group_by(NORDEMP, ANIO) %>%
      summarise(across(all_of(columnas_macro), suma_con_na),
                n_establecimientos = n(),
                .groups = "drop") %>%
      mutate(
        empleo_total      = rowSums(across(all_of(c(cols_obreros, cols_administrativos, cols_profesionales))), na.rm = TRUE),
        empleo_permanente = rowSums(across(all_of(cols_permanente)), na.rm = TRUE),
        empleo_temporal   = rowSums(across(all_of(cols_temporal)), na.rm = TRUE),
        participacion_permanente = ifelse(empleo_total > 0, 100 * empleo_permanente / empleo_total, NA),
        tamano_empresa    = case_when(empleo_total < 50 ~ "Pequena", empleo_total < 200 ~ "Mediana", TRUE ~ "Grande"),
        obreros_permanentes         = rowSums(across(all_of(c("C4R2C1", "C4R2C2"))), na.rm = TRUE),
        administrativos_permanentes = rowSums(across(all_of(c("C4R2C3", "C4R2C4"))), na.rm = TRUE),
        profesionales_permanentes   = rowSums(across(all_of(c("C4R1C3N", "C4R1C4N", "C4R2C3E", "C4R2C4E"))), na.rm = TRUE),
        temporal_directo  = rowSums(across(all_of(cols_temp_directo)), na.rm = TRUE),
        temporal_agencias = rowSums(across(all_of(cols_temp_agencias)), na.rm = TRUE),
        aprendices        = rowSums(across(all_of(cols_aprendices)), na.rm = TRUE)
      ) %>%
      transmute(NORDEMP, ANIO, tamano_empresa, empleo_total, empleo_permanente, empleo_temporal,
                participacion_permanente, VALORVEN,
                costos_totales_personal_total_c3r10c3 = C3R10C3,
                sueldos_permanentes_obreros_c3r2c1 = C3R2C1,
                sueldos_permanentes_administrativos_c3r2c2 = C3R2C2,
                sueldos_permanentes_profesionales_c3r2pt = C3R2PT,
                obreros_permanentes, administrativos_permanentes, profesionales_permanentes,
                temporal_directo, temporal_agencias, aprendices, n_establecimientos)
  }
  
  # Comprobación: reconstruimos 2019 y 2021 y comparamos con el panel
  variables_a_comparar <- c("empleo_total", "empleo_permanente", "empleo_temporal", "VALORVEN",
                            "costos_totales_personal_total_c3r10c3", "sueldos_permanentes_obreros_c3r2c1",
                            "obreros_permanentes")
  
  comprobacion <- tibble()
  for (anio in c(2019, 2021)) {
    reconstruido <- construir_anio(anio)
    original <- panel %>% filter(ANIO == anio) %>% select(NORDEMP, ANIO, all_of(variables_a_comparar))
    cruce <- inner_join(original, reconstruido, by = c("NORDEMP", "ANIO"), suffix = c("_panel", "_macro"))
    for (variable in variables_a_comparar) {
      x <- cruce[[paste0(variable, "_panel")]]
      y <- cruce[[paste0(variable, "_macro")]]
      iguales <- (is.na(x) & is.na(y)) | (!is.na(x) & !is.na(y) & abs(x - y) <= pmax(0.5, 1e-6 * abs(x)))
      comprobacion <- bind_rows(comprobacion, tibble(
        anio = anio, variable = variable,
        firmas_en_panel = nrow(original), firmas_reconstruidas = nrow(reconstruido), firmas_cruzadas = nrow(cruce),
        porcentaje_iguales = round(100 * mean(iguales), 2)
      ))
    }
  }
  ver(comprobacion, filas = 30)
  
  if (any(comprobacion$porcentaje_iguales < 99) || any(comprobacion$firmas_cruzadas < 0.99 * comprobacion$firmas_en_panel)) {
    mensaje <- "La reconstrucción de 2019/2021 NO coincide con el panel en al menos una variable (ver tabla arriba)."
    if (DETENER_SI_NO_COINCIDE) stop(mensaje, " No se agrega 2020.")
    cat("\nATENCIÓN:", mensaje, "Se sigue porque DETENER_SI_NO_COINCIDE = FALSE.\n")
  } else {
    cat("\nLa reconstrucción coincide con el panel: podemos construir 2020 con confianza.\n")
  }
  
  # Construimos 2020 y lo pegamos al panel (las demás columnas quedan vacías en 2020)
  filas_2020 <- construir_anio(2020)
  cat("Firmas en 2020:", nrow(filas_2020), "\n")
  panel <- bind_rows(panel, filas_2020) %>% arrange(NORDEMP, ANIO)
  
}

cat("Panel de firmas (con 2020):", nrow(panel), "filas,", n_distinct(panel$NORDEMP), "firmas\n")
...
guardar_tabla(comprobacion, "T00b_comprobacion_reconstruccion_2020", carpeta = CARPETA_VALIDACIONES,
              "Tabla 0b. Reconstrucción de 2019 y 2021 desde la macrobase frente al panel (% de firmas iguales)", decimales = 2)
...
datos <- panel %>%
  ...
  mutate(
    ...
    periodo     = ifelse(post == 1, "Después (2023-2024)", "Antes (2015-2022, con 2020)"),
...
tabla_2x2 <- datos %>%
  ...
  mutate(
    cambio_salario_pct = 100 * (`salario_promedio_Después (2023-2024)` / `salario_promedio_Antes (2015-2022, con 2020)` - 1),
    cambio_empleo_pct  = 100 * (`empleo_promedio_Después (2023-2024)` / `empleo_promedio_Antes (2015-2022, con 2020)` - 1)
  )
...
grafico_evol_salario <- ggplot(evolucion, aes(x = ANIO, y = salario_vs_2022, color = exposicion)) +
  ...
  labs(title = "Salario promedio por grupo de exposición",
       ...
       caption = "Promedios simples, sin controles. Incluye 2020.") +
  tema_tesis
...
#
# Creamos una dummy de pandemia (2020 y 2021) y otra
# de subsidios (2020 a 2023) y las metemos en un modelo con efecto fijo de año.
...
datos_demostracion <- datos %>%
  mutate(pandemia  = as.integer(ANIO %in% 2020:2021),
         subsidios = as.integer(ANIO %in% 2020:2023))
...
estudio_evento <- function(variable) {
  ...
  p_antes <- wald(modelo, keep = "ANIO_F::(2015|2016|2017|2018|2019|2020|2021):", print = FALSE)$p
...
NOTA_MODELO <- paste("Controles: firma, año, sector x año, tamaño x año y departamento x año (fijados en 2022).",
                     "\nIntervalos de confianza al 95%, errores agrupados por firma. Incluye 2020.")
