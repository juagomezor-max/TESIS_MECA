...
steps <- list(
  list(
    ...
    script = file.path(paths$scripts, "pipeline", "analisis_eam_eac.R")
  ),
  list(
    label = "2/4 Construccion del diccionario maestro",
    env = character(),
    script = file.path(paths$scripts, "pipeline", "construir_diccionario_maestro.R")
  ),
  list(
    label = "3/4 Construccion de macrobase EAM",
    env = character(),
    script = file.path(paths$scripts, "pipeline", "construir_macro_base_eam.R")
  ),
  list(
...
message("\n", steps[[4]]$label)
source(file.path(paths$scripts, "pipeline", "diagnostico_panel_nordemp_eam.R"), echo = FALSE, local = new.env(parent = globalenv()))
source(file.path(paths$scripts, "pipeline", "descriptivo_exposicion_eam.R"), echo = FALSE, local = new.env(parent = globalenv()))
